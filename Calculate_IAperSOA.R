...
for (target_img_id in all_image_ids) {
  ...
  original_img_df = original_img_df %>% filter(soa == i)
  ...
  for (other_image_index in 1:length(all_other_image_dfs)) {
    ...
    other_image_df = other_image_df %>% filter(soa == i)
    #other_image_df_filter_soa <- other_image_df[soa == target_soa, ]
    other_image_dfs[[other_image_index]] <- other_image_df
  }
  
  type1auc_df <- img_df[, .(type1auc = calculate_auc_img_sbj_word_perSOA(img_df, other_image_dfs, target_img_id, as.String(stem_word), sbj_idx, soa, output_plot = FALSE, include_confidence = FALSE)), 
                        by=c("sbj_idx", "stem_word", "soa")]
  
  type1auc_df_avg <- type1auc_df[, .(avg_type1auc = mean(type1auc)), by=c("stem_word", "soa")][order(-avg_type1auc)]
  #type1auc_df_avg <- type1auc_df_avg[avg_type1auc > 0.5, ]
  
  type1auc_df_avg$img_id <- target_img_id
  type1auc_df_avg$soa = i
  all_img_type1auc <- rbind(all_img_type1auc, type1auc_df_avg)
  
  print(sprintf("Finished image %s.", target_img_id))
  #return (type1auc_df_avg)
  }
}

IA_perSOA = all_img_type1auc
IA_perSOA = left_join(IA_perSOA, all_df %>% select(-avg_type1auc, -avg_type1auc_weighted), by = c('stem_word', 'soa', 'img_id'))
save(IA_perSOA, file = "IA_perSOA.RData")
